...
const AnswerController = {
    ...
    async getAnswerById(req, res) {
        try {
            ...
            if (!answerId) {
                return res.status(400).send({ message: "Falta el ID de la respuesta" });
            }

            const answer = await Answer.findById(answerId).populate("_idUser", "name image").select("_id reply _idUser");
            if (!answer) {
                return res.status(404).send({ message: "No se encontró la respuesta" });
            }

            res.status(200).send({ message: "Respuesta encontrada", answer });
        } catch (error) {
            console.error(error);
            res.status(500).send({ message: "Hubo un problema al obtener la respuesta" });
        }
    },
    ...
    async dislikeAnswer(req, res) {
        try {
            const { answerId } = req.params;
            const updateAnswer = await Answer.findByIdAndUpdate(answerId, { $inc: { likes: -1 } }, { new: false });
            ...
            console.error(error);
            res.status(500).send({ message: "Hubo un problema al dar dislike a la respuesta" });
        }
    },
